...
export default {
  ...
  computed: {
    displayedTags() {
      ...
      return this.showAllTags
        ? this.processedTags
        : this.processedTags.slice(0, this.maxVisibleTags);
    },
    hasMoreTags() {
      return this.processedTags.length > this.maxVisibleTags;
    },
    hiddenTagsCount() {
      return this.processedTags.length - this.maxVisibleTags;
    },
  ...
  methods: {
    ...
    async processTags() {
      // If tags are already full objects with tag_name, use them directly
      if (this.tags.length > 0 && this.tags[0].tag_name) {
        this.processedTags = this.tags;
        return;
      }

      // If tags are IDs, fetch the full tag objects
      try {
        const response = await api.get("tag/");
        const allTags = response.data;
        this.processedTags = this.tags.map((tagId) => {
        ...
        // Fallback to showing tag IDs if fetch fails
        this.processedTags = this.tags.map((tagId) => ({
          id: tagId,
          tag_name: `Tag ${tagId}`,
          tag_description: "",
